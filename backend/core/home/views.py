        ...
        try:
            ...
            if generate_summary:
                ...
                try:
                    ...
                    logger.info(f"File content type: {type(file_content)}, Size: {len(file_content)}")
                    
                    # Check if it's a PDF file by examining content
                    is_pdf = file_content[:5] == b'%PDF-' or file_name.lower().endswith('.pdf')
                    ...
                    if is_pdf:
                        ...
                        # Try basic UTF-8 decoding for unknown types
                    ...
                    if not file_text or len(file_text.strip()) == 0:
                        ...
                        # Truncate text if too large to avoid API limit
                        max_chars = 4000
                        ...
                        # Try RAG-based summary generation first
            ...
            try:
                ...
                if pinata_response.status_code != 200:
                    ...
                    return Response({'error': f'Failed to upload to IPFS: {pinata_response.text}'}, 
                                  status=status.HTTP_500_INTERNAL_SERVER_ERROR)
                ...
                logger.error(traceback.format_exc())
                return Response({'error': f'IPFS upload error: {str(e)}'}, 
                              status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            ...
            try:
                ...
                estimated_gas = contract.functions.addPatientDocument(
                    pat.id, patd.id, str(hashcid), current_timestamp, generate_summary
                ...
                logger.error(traceback.format_exc())
                return Response({'error': f'Blockchain error: {str(e)}'}, 
                              status=status.HTTP_500_INTERNAL_SERVER_ERROR)
